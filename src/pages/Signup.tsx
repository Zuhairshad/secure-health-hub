...
import { Shield, Eye, EyeOff, Heart, Activity, CheckCircle, ArrowLeft } from "lucide-react";
      ...
      <div className="min-h-screen flex flex-col lg:flex-row">
        ...
        <div className="relative lg:w-[48%] bg-[#0c1425] flex flex-col justify-between overflow-hidden min-h-[280px] lg:min-h-screen">
          ...
          >
            <Link to="/" className="flex items-center gap-3 group">
              ...
              <span className="text-white font-semibold text-lg tracking-tight">Secure Health Hub</span>
            </Link>
          </motion.div>
          ...
          <div className="relative z-10 flex-1 flex flex-col justify-center px-8 lg:px-10 pb-8 lg:pb-0">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.2 }}>
              ...
              <p className="mt-5 text-[15px] text-slate-400 leading-relaxed max-w-[380px]">
                Create your account to access a secure, HIPAA-compliant platform for managing your health records.
              </p>
            </motion.div>
          </div>
          ...
          >
            <p className="text-xs text-slate-500">© {new Date().getFullYear()} Secure Health Hub.</p>
        ...
        <div className="flex-1 flex items-center justify-center px-6 py-12 lg:py-0 bg-white">
          ...
          >
            ...
            <p className="mt-3 text-sm text-slate-500 leading-relaxed max-w-[340px] mx-auto">
              We've sent a confirmation link to <span className="font-medium text-slate-700">{email}</span>. Please verify your email to continue.
            </p>
            <Link to="/login" className="block mt-8">
              <Button
                variant="outline"
                className="w-full h-11 border-slate-200 hover:bg-slate-50 rounded-lg font-medium text-sm"
              >
        ...
        >
          ...
          <div className="text-center mb-8">
            ...
            <p className="mt-2 text-sm text-slate-500">
              Enter your details to get started with Secure Health Hub.
            </p>
          </div>
                    ...
                    {[1, 2, 3].map((level) => (
                      <div
                        ...
                        className={`h-1 flex-1 rounded-full transition-colors duration-300 ${
                          level <= strength.level ? strength.color : "bg-slate-100"
                        }`}
                      />
                    ))}
                  </div>
                  <span className="text-xs text-slate-400 font-medium">{strength.text}</span>
                </div>
              )}
            </div>
